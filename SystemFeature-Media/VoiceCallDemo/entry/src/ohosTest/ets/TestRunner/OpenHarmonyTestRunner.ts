...
export default class OpenHarmonyTestRunner implements TestRunner {
  ...
  async onRun() {
    ...
    hilog.info(DOMAIN, 'testTag', 'cmd : %{public}s', cmd);
    abilityDelegator.executeShellCommand(cmd,
      (err: any, d: any) => {
        hilog.info(DOMAIN, 'testTag', 'executeShellCommand : err : %{public}s', JSON.stringify(err) ?? '');
        hilog.info(DOMAIN, 'testTag', 'executeShellCommand : data : %{public}s', d.stdResult ?? '');
        hilog.info(DOMAIN, 'testTag', 'executeShellCommand : data : %{public}s', d.exitCode ?? '');
      });
